  ...
  const users = {
    async create(payload, tx) {
      ...
      const result = await run(
        executor,
        "INSERT INTO users (username, email, phone, created_at) VALUES (?, ?, ?, COALESCE(?, CURRENT_TIMESTAMP))",
        [payload.username, payload.email || null, payload.phone || null, createdAt || null]
      );
